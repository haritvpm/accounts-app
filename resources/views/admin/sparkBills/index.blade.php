...
@section('content')
@can('spark_bill_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.spark-bills.create') }}">
                ...
                <tbody>
                    @foreach($sparkBills as $key => $sparkBill)
                        <tr data-entry-id="{{ $sparkBill->id }}">
                            ...
                            <td>
                                @can('spark_bill_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.spark-bills.show', $sparkBill->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('spark_bill_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.spark-bills.edit', $sparkBill->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('spark_bill_delete')
                                    <form action="{{ route('admin.spark-bills.destroy', $sparkBill->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('spark_bill_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.spark-bills.massDestroy') }}",
