...
@can('spark_bill_data_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.spark-bill-datas.create') }}">
                        ...
                        <tr data-entry-id="{{ $sparkBillData->id }}">
                            ...
                            <td>
                                @can('spark_bill_data_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.spark-bill-datas.show', $sparkBillData->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('spark_bill_data_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.spark-bill-datas.edit', $sparkBillData->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('spark_bill_data_delete')
                                    <form action="{{ route('admin.spark-bill-datas.destroy', $sparkBillData->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('spark_bill_data_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.spark-bill-datas.massDestroy') }}",
