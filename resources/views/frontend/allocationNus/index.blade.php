        ...
        <div class="col-md-12">
            @can('allocation_nu_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.allocation-nus.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $allocationNu->id }}">
                                        ...
                                        <td>
                                            @can('allocation_nu_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.allocation-nus.show', $allocationNu->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('allocation_nu_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.allocation-nus.edit', $allocationNu->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('allocation_nu_delete')
                                                <form action="{{ route('frontend.allocation-nus.destroy', $allocationNu->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('allocation_nu_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.allocation-nus.massDestroy') }}",
